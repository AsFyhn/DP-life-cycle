...
def EGM_loop (sol,t,par):
    ...
    for i_a,a in enumerate(par.grid_a): # Loop over end-of-period assets
        ...
        m_next = par.R*a*par.eps                          # 1. Find m_next
        c_next = interp(m_next)                           # 2. Find c_next using interpolation of next period solution
        EU_next = np.sum(par.eps_w*marg_util(c_next,par)) # 3. Find expected marginal utility of next period consumption
        c_now = inv_marg_util(par.beta*par.R*EU_next, par)     # 4. Find optimal consumption using inverted Euler
