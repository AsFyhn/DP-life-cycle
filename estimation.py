...
class SMD:
    ...
    def obj_function(self,theta,est_par,W):
        ...
        sim = self.simulator.simulate(sim, self.model, sol)
        ...
        self.diff = self.mom_data.C_avg - self.mom_sim
        self.obj  = (np.transpose(self.diff) @ W) @ self.diff

        return self.obj 

    def estimate(self,theta0,est_par,W):
        assert(len(W[0])==len(self.mom_data.C_avg)) # check dimensions of W and mom_data
